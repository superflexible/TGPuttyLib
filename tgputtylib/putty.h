...
/* TG    X(FILENAME, NONE, xauthfile) \
	...
	/*
	 * TG, from PuTTY version history regarding sshbug_dropstart:                                                      \
	 * Bug workaround flag to wait for the server's SSH greeting      \
	 * before sending our own, for servers (or proxies) that lose     \
	 * outgoing data before seeing any incoming data.                 \
	*/ \
