 ...
 */
extern NORMALCODE(const) char * NORMALCODE(const) appname; // TG: make it variable and settable in DLL
...
struct ConfKeyInfo {
	...
	#ifdef TGDLL
	const char *id_keyword; // TG: added this for TGPuttyLib
	#endif
};
...
extern void pktin_free_queue_callback(void *vctx);
...
// TG: size of the per-context diagnostic print line-buffer (printbuf below).
// Must match the printbufsize used in psftp.c (which #defines it to this).
#define TGPRINTBUFSIZE 300
...
typedef struct
{
  ...
  // RESERVED PART - INTERNAL USE ONY
  LogContext *psftp_logctx;
  Backend *backend;
  Conf *conf;
  bool sent_eof;

  // additional static items from psftp.c
  __int64 bufchainlength;
  bufchain received_data;

  // TG stuff
  char *caller_supplied_password;
  bool tried_caller_supplied_password_once;

  // fields from callback.c
  ...
  // from sshrand.c
  int random_active;
  struct random_timer_context { int dummy; } random_timer_ctx;
  prng *global_prng;
...
#ifdef _WINDOWS
  // from winselcli.c
  tree234 *winselcli_sockets;
  HANDLE winselcli_event;
  ...
  // from agent-client.c
  tree234 *agent_pending_queries;

  // from fd-socket.c
  tree234 *fdsocket_by_outfd;
  tree234 *fdsocket_by_infd;
  tree234 *fdsocket_by_inerrfd;
  // from handle-wait.c
  tree234 *handlewaits_tree_real;

  // TG: raw SSH (rsync) support - Milestone 1 (one exec channel per connection)
  bool ssh_raw_mode;          // true => exec a plain command instead of the SFTP subsystem
  bool ssh_capture_stderr;    // true => route stderr into ssh_stderr_data instead of local stderr
  char *ssh_exec_command;     // command to exec on the (main) session channel in raw mode
  bufchain ssh_stderr_data;   // captured stderr bytes, drained by tgssh_recv

  // TG: raw SSH (rsync) support - Milestone 2 (persistent connection, channel per command)
  bool ssh_no_channel;        // true => connect without opening any session channel
  tree234 *ssh_channels;      // open tgssh_channel objects, keyed by pointer

  // TG: per-context diagnostic print line-buffer. Was a THREADVAR in psftp.c;
  // moved here so that when one thread drives two contexts (e.g. copying from
  // one server to another), a partial (newline-less) log line buffered by one
  // context can never be concatenated onto / mis-attributed to the other.
  char printbuf[TGPRINTBUFSIZE];
  size_t printbufpos;
...
// we will keep asserts even in release versions!
// they are very important
#undef assert
#define assert(cond) ((cond) ? (void)0 : tgdll_assert(#cond, __FILE__, __LINE__))
#endif
