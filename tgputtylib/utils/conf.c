...
static void copy_value(struct value *to, struct value *from, int type)
{
    switch (type) {
      ...
      case TYPE_FILENAME:
        ...
        break;
      case TYPE_FONT:
        NORMALCODE(to->u.fontval = fontspec_copy(from->u.fontval);)
        break;
    }
}
    ...
    for (i = 0; (entry = index234(conf->tree, i)) != NULL; i++) {
        ...
        switch (valuetypes[entry->key.primary]) {
          ...
          case TYPE_FILENAME:
            ...
            break;
		  case TYPE_FONT:
			NORMALCODE(fontspec_serialise(bs, entry->value.u.fontval);)
            break;
        }
    }
    ...
    while (1) {
        ...
        switch (valuetypes[entry->key.primary]) {
          ...
          case TYPE_FILENAME:
            ...
            break;
          case TYPE_FONT:
            NORMALCODE(entry->value.u.fontval = fontspec_deserialise(src));
            break;
        }
