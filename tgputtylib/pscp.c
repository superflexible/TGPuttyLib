...
#include <time.h>
#include <assert.h>
#include <setjmp.h> /* TG: must come BEFORE putty.h - putty.h redefines
                       printf/fprintf/fwrite/fflush as macros, which must
                       not be in effect while system headers are parsed */
...
#ifdef TGDLL
/* ====================================================================
 * TG: scp-over-channel support.
 *
 * In the DLL, pscp.c is not a standalone tool: it contributes ONLY its
 * scp protocol engine (response(), scp_source_* / scp_send_* / scp_sink_* /
 * scp_get_sink_action/scp_recv_filedata/...), which runs over a session
 * channel of the host's existing persistent SSH connection (the same
 * machinery as tgssh_open_channel in psftp.c). The tool parts - main(),
 * its own backend/seat, command line, local file recursion - are
 * compiled out below with #ifndef TGDLL.
 *
 * The protocol engine's I/O funnels through exactly these primitives,
 * which are redirected onto the current channel:
 *   backend_send(backend, ...)     -> tgscp_chan_send()
 *   backend_sendbuffer(backend)    -> tgscp_chan_sendbuffer()
 *   ssh_scp_recv(buf, len)         -> channel drain (TG version below)
 *   bump(...)                      -> longjmp back to the EXPORT wrapper
 *
 * All operation state is THREADVAR: an scp operation is a single
 * synchronous DLL call, so one thread never has two operations in
 * flight even when it alternates between two library contexts.
 * (setjmp.h is included at the top of the file, before putty.h.)
 * ==================================================================== */

static THREADVAR void *tgscp_channel;        /* tgssh channel handle */
static THREADVAR jmp_buf tgscp_bumpjmp;      /* bump() recovery point */
static THREADVAR bool tgscp_bumpjmp_valid;

static void tgscp_chan_send(const void *data, size_t len);
static size_t tgscp_chan_sendbuffer(void);
#endif

#ifdef TGDLL
/* TG: per-operation protocol state. THREADVAR (not plain static) so two
 * library contexts on different threads can run scp concurrently; see the
 * banner comment above for why THREADVAR is sufficient. The tool-only
 * flags keep their stock defaults and are never changed in DLL mode. */
static THREADVAR bool list;
static THREADVAR bool verbose;
static THREADVAR bool recursive;
static THREADVAR bool preserve;
static THREADVAR bool targetshouldbedirectory;
static THREADVAR bool statistics;
static THREADVAR int prev_stats_len;
static THREADVAR bool scp_unsafe_mode;
static THREADVAR int errs;
static THREADVAR bool try_scp;
static THREADVAR bool try_sftp;
static THREADVAR bool main_cmd_is_sftp;
static THREADVAR bool fallback_cmd_is_sftp;
static THREADVAR bool using_sftp;
static THREADVAR bool uploading;

static Backend *backend; /* unused in DLL mode; the macros below bypass it */
static Conf *conf;
static THREADVAR bool sent_eof;

/* Redirect the engine's two send primitives onto the current channel.
 * (Must come after the variable declarations so the macro argument
 * 'backend' still parses; the variable itself is never dereferenced.) */
#define backend_send(be, d, l) tgscp_chan_send(d, l)
#define backend_sendbuffer(be) tgscp_chan_sendbuffer()
#else
...
#ifndef TGDLL /* TG: provided by psftp.c / not needed in the DLL */
void ldisc_echoedit_update(Ldisc *ldisc) { }
void ldisc_check_sendok(Ldisc *ldisc) { }
#endif

#ifndef TGDLL /* TG: the DLL has no pscp seat - data arrives via the channel */
...
#else /* TGDLL */
/* TG: all user-facing text goes through the printf redirection in putty.h,
 * which delivers it to the host's printmessage_callback. */
static void tell_str(FILE *stream, const char *str)
{
    printf("%s", str);
}
...
#ifndef TGDLL /* TG: original seat-fed receive path (tool mode only) */
...
/* TG: receive exactly len bytes of the channel's stdout, pumping the event
 * loop. stderr arriving in between is relayed to the host as messages.
 * Honors libctx->aborted and libctx->timeoutticks (inactivity). */
static bool ssh_scp_recv(void *vbuf, size_t len)
{
    char *buf = (char *)vbuf;
    char errbuf[4096];
    ...
    while (len > 0) {
        int outlen = (len > (size_t)INT_MAX) ? INT_MAX : (int)len;
        int errlen = sizeof(errbuf)-1;

        tgssh_channel_recv(tgscp_channel, buf, &outlen, errbuf, &errlen,
                           curlibctx);
        if (errlen > 0 && curlibctx->printmessage_callback) {
            errbuf[errlen] = 0;
            curlibctx->printmessage_callback(errbuf, 1, curlibctx);
        }
        if (outlen > 0) {
            buf += outlen;
            len -= outlen;
            starttick = TGGetTickCount64(); /* progress - restart the clock */
            continue;
        }

        if (curlibctx->aborted)
            return false;
        if (!tgssh_channel_is_open(tgscp_channel, curlibctx))
            return false; /* closed and drained */

        /* Pump (also processes window adjusts for our sends). */
        if (!tgssh_channel_can_recv(tgscp_channel, 1000, curlibctx)) {
            int timeoutms = curlibctx->timeoutticks;
            if (timeoutms < 1000)
                timeoutms = 60000;
            if ((int64_t)(TGGetTickCount64() - starttick) > timeoutms)
                return false; /* inactivity timeout */
        }
    }
...
/* TG: protocol failure - report, then jump back to the EXPORT wrapper
 * (which closes the channel and returns an error code). */
static NORETURN PRINTF_LIKE(1, 2) void bump(const char *fmt, ...)
{
    char *str;
    va_list ap;
    ...
    if (tgscp_bumpjmp_valid)
        longjmp(tgscp_bumpjmp, 1);

    /* No recovery point - should not happen. Hand the failure to the host
     * as an exception rather than taking the process down. */
    if (curlibctx && curlibctx->raise_exception_callback)
        curlibctx->raise_exception_callback("scp protocol failure",
                                            __FILE__, __LINE__, curlibctx);
    /* satisfy NORETURN even if no callback is installed */
    for (;;) ;
}
...
#else
/* TG: messages go to the host application, no terminal to protect - pass
 * the string through unmodified (and don't free it: it isn't a copy). */
#define with_stripctrl(varname, input)                                  \
...
#endif /* !TGDLL - end of tool-only connection/stats/cmdline-path section */
...
#ifndef TGDLL /* TG: psftp.c provides these in the DLL */
...
#endif /* !TGDLL - end of duplicate/listing section */
 ...
 */
#ifndef TGDLL /* TG: tool-only from here on - local-file drivers, wildcards,
                 command line and main(). The DLL exports follow at the end
                 of the file. */
...
#endif /* !TGDLL - end of tool-only tail */

#ifdef TGDLL
/* ====================================================================
 * TG: exported scp operations over the persistent SSH connection.
 *
 * Both functions open one session channel on the host's existing
 * connection (the same one the tgssh_* listing/command functions use),
 * run PuTTY's scp protocol engine on it, and close the channel again -
 * the SSH connection stays up. They therefore require a connection
 * made with tgssh_connect_persistent (or any live tgssh connection).
 *
 * Data is exchanged with the host application through the established
 * libctx stream callbacks, sequentially from offset 0:
 *   download: write_to_stream(offset, buf, len)
 *   upload:   read_from_stream(offset, buf, len)
 * Progress and cancellation: progress_callback(bytes, isupload) - a
 * false return aborts the transfer - plus libctx->aborted.
 * Diagnostics and remote scp error text go to printmessage_callback.
 *
 * Returns 0 on success; nonzero error codes below.
 * ==================================================================== */
...
#define TGSCP_ERR_NOTCONNECTED   -1  /* could not open a channel */
#define TGSCP_ERR_PROTOCOL       -2  /* scp protocol failure (bump) */
#define TGSCP_ERR_REMOTE         -3  /* remote scp reported an error */
#define TGSCP_ERR_STREAM         -4  /* host stream callback failed */
#define TGSCP_ERR_ABORTED        -5  /* aborted by host */
#define TGSCP_ERR_UNEXPECTED     -6  /* unexpected protocol record */

/* Channel-backed send primitives the engine macros route to. */
static void tgscp_chan_send(const void *data, size_t len)
{
    if (len > 0)
        tgssh_channel_send(tgscp_channel, data, (int)len, curlibctx);
}

static size_t tgscp_chan_sendbuffer(void)
{
    return (size_t)tgssh_channel_sendbuffer(tgscp_channel, curlibctx);
}

/* Single-quote a path for the remote shell: ' -> '\'' inside quotes. */
static char *tgscp_quote(const char *path)
{
...
/* Common per-operation setup: reset protocol state, open the channel. */
static bool tgscp_begin(const char *cmd, TTGLibraryContext *libctx)
{
    curlibctx = libctx;
    errs = 0;
    using_sftp = false;   /* we always talk SCP1 over our own channel */
    recursive = false;
    uploading = false;
    sent_eof = false;
    tgscp_bumpjmp_valid = false;

    tgscp_channel = tgssh_open_channel(cmd, libctx);
    return tgscp_channel != NULL;
}

static void tgscp_end(TTGLibraryContext *libctx)
{
    tgscp_bumpjmp_valid = false;
    if (tgscp_channel) {
        tgssh_channel_close(tgscp_channel, libctx);
        tgscp_channel = NULL;
    }
}

/*
 * Download one file via scp. The remote path is quoted here - pass it
 * unescaped. Outputs (each pointer may be NULL): the file size, the
 * mtime from the server's T record (0 if the server sent none) and the
 * permissions from the C record. File data goes to write_to_stream.
 */
EXPORT int tgscp_download(const char *remotepath,
                          ...
                          TTGLibraryContext *libctx)
{
    ...
    /* -p requests T records so we can report the remote timestamp. */
    quoted = tgscp_quote(remotepath);
    cmd = dupprintf("scp -p -f %s", quoted);
    sfree(quoted);
    ...
    scp_sink_init(); /* sends the initial ack */

    while (1) {
        if (scp_get_sink_action(&act)) {
            /* finished (or remote error already counted in errs) */
            result = errs ? TGSCP_ERR_REMOTE : TGSCP_ERR_UNEXPECTED;
            break; /* we never got our file */
        }
        if (act.action == SCP_SINK_RETRY)
            continue;
        if (act.action != SCP_SINK_FILE) {
            /* directories need recursive mode, which we did not request */
            result = TGSCP_ERR_UNEXPECTED;
            break;
        }

        if (osize) *osize = act.size;
        if (omtime) *omtime = act.settime ? (uint64_t)act.mtime : 0;
        if (operms) *operms = (int)act.permissions;

        scp_accept_filexfer();

        while (received < act.size) {
            char dbuf[32768];
            uint64_t rem = act.size - received;
            int chunk = rem > sizeof(dbuf) ? (int)sizeof(dbuf) : (int)rem;
            int got = scp_recv_filedata(dbuf, chunk);
            if (got <= 0) {
                result = TGSCP_ERR_PROTOCOL;
                break;
            }
            if (curlibctx->write_to_stream) {
                if (curlibctx->write_to_stream(received, dbuf, got,
                                               curlibctx) != got) {
                    result = TGSCP_ERR_STREAM;
                    break;
                }
            }
            received += got;
            if (curlibctx->aborted ||
                (curlibctx->progress_callback &&
                 !curlibctx->progress_callback(received, false, curlibctx))) {
                result = TGSCP_ERR_ABORTED;
                break;
            }
        }
        if (result == 0) {
            if (scp_finish_filerecv())
...
/*
 * Upload one file via scp. remotedir is the target DIRECTORY (quoted
 * here - pass it unescaped); filename is the file name for the scp
 * C record (no path components). size must be known up front (scp
 * protocol requirement). mtime 0 = don't send a T record. permissions
 * e.g. 0644; pass <0 for the default. Data comes from read_from_stream.
 */
EXPORT int tgscp_upload(const char *remotedir, const char *filename,
                        const uint64_t size, const uint64_t mtime,
                        ...
                        TTGLibraryContext *libctx)
{
    ...
    /* reads the sink's initial ready-ack (the SCP1 branch of setup) */
    if (scp_source_setup(remotedir, true)) {
        result = TGSCP_ERR_REMOTE;
        goto out;
    }

    if (mtime != 0) {
        if (scp_send_filetimes((unsigned long)mtime, (unsigned long)mtime)) {
            result = TGSCP_ERR_REMOTE;
            goto out;
        }
    }

    if (scp_send_filename(filename, size, permissions)) {
        result = TGSCP_ERR_REMOTE;
        goto out;
    }
    ...
    while (sent < size) {
        char dbuf[32768];
        uint64_t rem = size - sent;
        int chunk = rem > sizeof(dbuf) ? (int)sizeof(dbuf) : (int)rem;
        int got = curlibctx->read_from_stream ?
                  curlibctx->read_from_stream(sent, dbuf, chunk, curlibctx) : 0;
        if (got <= 0) {
            /* source stream ended early - the remote now expects more
             * bytes than we can deliver; abandon via channel close */
            result = TGSCP_ERR_STREAM;
            goto out;
        }
        if (scp_send_filedata(dbuf, got)) {
            result = TGSCP_ERR_PROTOCOL;
            goto out;
        }
        sent += got;
        if (curlibctx->aborted ||
            (curlibctx->progress_callback &&
             !curlibctx->progress_callback(sent, true, curlibctx))) {
            result = TGSCP_ERR_ABORTED;
            goto out;
        }
    }
    ...
    /* clean half-close so the remote scp exits */
    tgssh_channel_send_eof(tgscp_channel, libctx);
