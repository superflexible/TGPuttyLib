...
void sgrowstringarray(const char ***arr,size_t *s,const size_t n) // TG
{
   void **l = (void **) *arr;
   l = safegrowarray(l,s,n,sizeof(*l),1,false);
   *arr = (const char **) l;
}
  ...
  begin_key_exchange:
    ...
    if (s->warn_hk) {
        ...
        for (j = 0; j < s->n_uncert_hostkeys; j++) {
            ...
            if (better) {
				sgrowstringarray(&betteralgs, &bettersize, nbetter); // TG: fix compiler error
        ...
        if (betteralgs) {
			...
			sfree(((void*)betteralgs)); // TG: fix compiler error
        } else {
