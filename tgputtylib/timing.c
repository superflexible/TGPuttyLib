...
#ifdef TGDLL
#define timers (curlibctx->timers)
#define timer_contexts (curlibctx->timer_contexts)
#define snow (curlibctx->snow)
#else
static tree234 *timers = NULL;
static tree234 *timer_contexts = NULL;
static unsigned long snow = 0L; // TG 2019: renamed "now" to "snow"
#endif
...
{
    /*
     * Return the last value we stored in 'snow'. In particular,
