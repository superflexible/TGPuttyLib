...
{
#ifdef TGDLL
	tgdll_fwrite(data, 1, len, stdout); // stdout means: send to host application
#else
										...
										strbuf *sb)
{
	...
	if (curlibctx && curlibctx->get_input_callback(buf,lenof(buf)-1,curlibctx))
	{
	   nread=strnlen(buf,4095);
    ...
    void (*callback)(void *ctx, SeatPromptResult result), void *ctx)
{
	...
	if (curlibctx && curlibctx->get_input_callback) // TG
	{
	   char line[100]; // TO DO TO TEST
