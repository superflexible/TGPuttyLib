...
#ifdef TGDLL
#define winselcli_sockets (curlibctx->winselcli_sockets)
#else
...
void winselcli_setup(void)
{
	...
	if (p_WSAEventSelect &&
		((winselcli_event == INVALID_HANDLE_VALUE) ||
		 // TG: need to check against 0 too because curlibctx->winselcli_event
		 // isn't necessarily initialized to INVALID_HANDLE_VALUE
		 (winselcli_event == 0)))
