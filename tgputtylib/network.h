...
void sk_init(void);                    /* called once at program startup (TG: actually, once per thread) */
void sk_cleanup(const bool cleanupglobalstoo);  /* called just before program exit (TG: and once per thread) */
